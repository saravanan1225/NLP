...
def audio1():
	...
	c=r"D:\Streamlit app\Sentimental Analysis final\Stream" # Path to the Audio_File directory (Python 3.7)
	...
	if choice !="---------------------------------------------------------SELECT THE LANGUAGE----------------------------------------------------------":
		...
		if video is not None:
			if video.type=="video/mp4":
				...
				audio = video1.audio
				audio.write_audiofile(r"D:\Streamlit app\Sentimental Analysis final\Stream\sample.wav")
				...
				envelope();
				st.write('Audio Noises and Voices Masked')
				path="sample.wav"
				sound = AudioSegment.from_wav(path)  
				chunks = split_on_silence(sound,
			        # experiment with this value for your target audio file
...
def envelope( ):
    ...
    for mean in y_mean:
        if mean>threshold:
            ...
            mask.append(False)
    wavfile.write(r'C:\Users\saravanan\Desktop\sentimental analysis project\\'+str(file), rate=rate,data=y[mask])       
    return mask
...
def img(senti):
    ...
    if senti=="pos":
        document.append(Image('pos.png',2.2*inch,2.2*inch))
        document.append(Spacer(1,50))
    elif senti=="neg":
        document.append(Image('neg.png',2.2*inch,2.2*inch))
        document.append(Spacer(1,50))
    elif senti=="neu":
    	document.append(Image('neu.png',2.2*inch,2.2*inch))
    	document.append(Spacer(1,50))
...
def addParagraph():
	addTitle(document)
	document.append(Paragraph('Paragraph :',ParagraphStyle(name='Name',fontFamily='Times New Roman',fontSize=25,alignment=TA_JUSTIFY)))
	...
	if choice !="---------------------------------------SELECT THE LANGUAGE-----------------------------------":
		...
		if video is not None:
			if video.type=="video/mp4":
				...
				plot_spectrogram();
				wit();
				pas=(mysp.myspgend("sample",r"D:\Streamlit app\Sentimental Analysis final\Stream"))
				if pas[2]==0:
	...
	if choice == "Home":
		...
		if st.sidebar.checkbox("Login"):
			...
			if result:
				...
				st.sidebar.subheader(' PLEASE SELECT THE OPTION :sunglasses:')
